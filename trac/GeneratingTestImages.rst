Sometimes it's nice to have a file of a specific size or pixel type for
testing. To generate a file (that contains gradient images):

::

    touch "my-special-test-file&pixelType=uint8&sizeZ=1&sizeC=1&sizeT=1&sizeX=8192&sizeY=8192.fake"
...
support for those files is disabled and so you'll need to convert the
file to something else. Make sure that you have Bio-Formats built and
the JARs in your CLASSPATH (individual JARs or just loci\_tools.jar):

::

    bfconvert my-special-test-file&pixelType=uint8&sizeZ=1&sizeC=1&sizeT=1&sizeX=8192&sizeY=8192.fake my-special-test-file.tiff

If you don't have the command line tools installed, substitute
'loci.formats.tools.ImageConverter?' for 'bfconvert'.
