...
``jenkins`` alias for the server, e.g. ``:jenkins: `Jenkins server <>``` or
``jenkinsjob`` alias for the OMERO job, e.g. ``:jenkinsjob:`here 
<lastSuccessfulBuild/>``` or ``javadoc`` alias for the Javadoc generated by 
the OMERO job, e.g. ``:jenkinsjob:`here <lastSuccessfulBuild/>```.
